...
- **Acceptance**:
  ...
  - 例外をスローしない
- **Status**: ✅ 完了 - `lib/s3/delete.ts`実装、ベストエフォート削除パターン適用
...
- **Status**: ✅ 完了 - `lib/s3/delete.test.ts`実装、6個のテストケース全パス
...
- **Status**: ✅ 完了 - EmployeeFormに写真選択UI、プレビュー、変更/削除ボタン、確認ダイアログを実装、17個のテストケース全パス
...
- **Status**: ✅ 完了 - `validatePhotoFile`を統合、ファイル選択時の即座バリデーション実装、エラーメッセージ表示機能追加、24個のテストケース全パス
...
- **Status**: ✅ 完了 - `uploadToS3`関数実装、`handleSaveClick`でアップロード→フォーム送信フロー実装、プログレスインジケーター、エラーハンドリング（401/403/500/ネットワークエラー）完全実装、10個の新規テストケース全パス、既存テスト24個も全パス、全434テスト合格
