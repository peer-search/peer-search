...
- [x] 5.1 サンプルデータの投入
  ...
  - _完了: サンプルデータ投入スクリプト (scripts/seed-organizations.ts) とRPC関数テストスクリプト (scripts/test-rpc-hierarchy.ts) を作成。package.jsonにコマンドを追加。_
...
- [x] 5.2 トップページ表示の動作確認
  ...
  - _完了: Chrome DevTools MCPで確認。4階層のカード内包レイアウトが正しく表示される_

- [x] 5.3 ナビゲーションの動作確認
  - 組織カードをクリックして`/employees?org_id={node.id}`へ遷移することを確認する
  - URLパラメータに組織IDが含まれていることを確認する
  - _Requirements: 4.1, 4.2, 4.3_
  - _完了: フロントエンドチームカードをクリックし、`/employees?org_id=e5e062b6-bb9a-40c6-9613-d4c9b48e92d0`へ正しく遷移_

- [x] 5.4 レスポンシブデザインの動作確認
...
- [x] 5.6* アクセシビリティの動作確認
  ...
  - _完了: Tabキーでフォーカス移動、フォーカスアウトライン表示、aria-label属性確認_
...
- [x] 5.7 パフォーマンスの確認
  ...
  - _完了: FCP 688ms、DOM Interactive 1069ms、Response Time 1060ms、Server Component確認_

### 6. 自動テストの実装（テストインフラ構築後に追加）
- [x] 6.1 テストフレームワークのセットアップ
  - Vitest、React Testing Library、happy-domをインストールする
  - vitest.config.tsとvitest.setup.tsを作成する
  - package.jsonにテストスクリプトを追加する（`test`, `test:run`, `test:ui`, `test:coverage`）
  - steering/tech.mdにテストフレームワークの情報を追加する

- [x] 6.2 ユニットテストの作成
  - `buildTree()`関数のテストを作成する（lib/organizations/tree.test.ts）
  - 空配列、単一ノード、親子関係、複数階層、兄弟ノード、ソート、データ不整合のケースをテストする

- [x] 6.3 コンポーネントテストの作成
  - `OrganizationCard`コンポーネントのテストを作成する（components/organization/organization-card.test.tsx）
  - 組織名表示、リンクURL、ARIA属性、子ノードレンダリングをテストする
  - `OrganizationCardList`コンポーネントのテストを作成する（components/organization/organization-card-list.test.tsx）
  - 空状態、単一/複数組織表示、階層構造、レイアウトクラスをテストする
